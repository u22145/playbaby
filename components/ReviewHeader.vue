  ...
  <div>
    <ul class="nav-bar" :class="[hide ? 'is-hide2' : 'not-hide2']">
        <div class="white-b">
          <back style="float:left;display:inline"/>
            回复 - {{this.$store.state.setReview.nickname}}
    <!-- {{this.$route.params.id}} -->
        ...
        <ul class="sticky-footer2" >
            <div>
                <input class="reply"  type="text" placeholder="請輸入留言" v-model="reply">
            </div>
            <button class="submit-btn2" type="button" @click="reviewSubmit2(1)">提交</button>
        </ul>
  </div>
</template>
<script>
...
export default {
  ...
  methods: {
    ...
    async reviewSubmit2(module) {
        let aaa = this.$route.params.id.split("|");
        console.log(aaa,"靠")
        let encryptData = await this.encrypt({"tid":aaa[1],"rev_id":aaa[0],"reply":this.reply},1)
        let formData = new FormData();
        formData.append("data", encryptData);
        this.$axios.post(process.env.BASE_URL+'blog/reply_write', formData)
            .then((res) => {
                console.log("可能487",res.data)
