<template>
  <div>
    <ul class="nav-bar2" :class="[hide ? 'is-hide2' : 'not-hide2']">
        <div class="white-b">
    ...
    <!-- {{this.$route.params.id}} -->
        ...
        <ul class="sticky-footer2" >
            ...
            <button class="submit-btn2" type="button" @click="reviewSubmit(1)">提交</button>
        </ul>
  </div>
</template>
<script>
...
export default {
  ...
  methods: {
    ...
    async reviewSubmit(module) {
        let encryptData = await this.encrypt({"module":module ,"tid":this.$route.params.id,"review":this.review},1)
