<template>
    <div class="my-outer">
        <!-- <div class="radius-header"><back style="float:left;display:inline"/><div class="align-center">{{title}}</div></div> -->
        <div class="my-container align-center post-page">
            <div class="title">發啪啪</div>
            <input type="file" id="upload-file" ref="file" v-on:change="uploadFile($event)" hidden>
            <label for="upload-file" class="upload-file">+</label>
            <textarea name="" id="" cols="30" rows="10" :placeholder="placeholder" v-model="content" style="padding:10px"></textarea>
             <div id="cover-img-upload"></div>
             <div>
                 <div>Baby幣收費</div>
                 <input class="charge" type="number" placeholder="收費" v-model="fee">
             </div>
            <div class="btn" :class="[disabledBtn ? 'disabled': '']" @click="submitPost">发布</div>
        </div>
    </div>
</template>
<script>

import '@/utils/mixins';
import aesJs from 'aes-js';
import * as qiniu from 'qiniu-js'
export default {
    data() {
        return {
            title: "发布啪啪",
            uploadType: 5,
            key: '128bitslength*@#',
            content: "",
            pic: "",
            video_id: "",
            blog_id: "",
            video_pic: "",
            apiSlug: "blog/write_new",
            ffmpegType: 0,
            placeholder: "记录这一刻,晒给懂你的人",
            fee: 0,
            disabledBtn: true,
            novelTitle: "",
            key: '128bitslength*@#',
            encryptFile: null,
            qiniuName: ""
        }
    },
    methods: {

        async submitPost() {
            if(this.disabledBtn) {
                this.$alert("档案上传还未完成 请耐心等候")
                return false
            }
            if((this.content == "") || (this.uploadType == 0 && this.title =='')) {
                this.$alert("请填写内容或标题")
                return false
            }
            this.disabledBtn = true

            let data = {
                title: this.content,
                ts: "",
                pic: this.video_pic,
                fee: this.fee,
                appid: 1,
                ffmpeg: 1
            }
            let encryptData = await this.encrypt(data)
            let formData = new FormData();
            formData.append("data", encryptData)
            this.$axios.post(process.env.BASE_URL+'papa/upload_new', formData)
                .then((res) => {
                        if(res.data.status ==1) {
                            
                            if(res.data.data.papa_id) {
                                this.papa_id = res.data.data.papa_id
                            }
                            this.uploadFFMpeg()
                            this.$alert("上传成功 请静待管理员审核")
                            .then(() => {
                                // this.$router.push('/')
                            })
                            
                        }
                    })
                    .catch((err) => {
                        console.log("get encrypt error")
                })
        },
        async uploadFile(e) {
        this.file = this.$refs.file.files[0];
        let data = {type: 8, appid:1}
            let encryptData = await this.encrypt(data)
            let formData = new FormData();
            formData.append("data", encryptData)
            formData.append("file[]", this.file)
            this.$axios.post(process.env.BASE_URL+'my/upload_pic', formData)
                .then((res) => {
                        if(res.data.status == 1) {
                            if(res.data.data[0].video_id) {
                                this.video_id = res.data.data[0].video_id
                            }
                            this.disabledBtn = false
                        }
                        
                        // this.video_pic = res.data.data.video_pic
                    })
                    .catch((err) => {
                        console.log("get encrypt error")
                    })
        },
        async getQiniuToken() {
                    let encryptData = await this.encrypt({})
                    let formData = new FormData();
                    formData.append("data", encryptData)
                    this.$axios.post(process.env.BASE_URL+'my/get_qiniu_token', formData)
                        .then((res) => {
                            console.log(res)
                            })
                            .catch((err) => {
                                console.log("get encrypt error")
                            })
        },
        async uploadFFMpeg() {
            let data = {
                appid: 1,
                type: 1,
                video_id: this.video_id,
                record_id: this.papa_id ? this.papa_id : this.blog_id
            }
            let encryptData = await this.encrypt(data)
            let formData = new FormData();
            formData.append("data", encryptData)
            this.$axios.post(process.env.BASE_URL+'my/ffmpeg_uploaded_video', formData)
                .then((res) => {
                    })
                    .catch((err) => {
                        console.log("get encrypt error")
                })
        }
    },
    mounted() {
        document.body.style.overflow = "visible"
        this.$store.commit("setPopup", false)
    }
}
</script>
<style lang="scss">
    .post-page {
        padding: 20px 20px 80px 20px;
        .upload-file {
            width: 70px;
            height: 70px;
            line-height: 70px;
            font-size: 24px;
            display: block;
            border: 3px  dotted #ccc;
        }
        textarea {
            border: 0;
            width: 100%;
            background:#efefef;
            border-radius: 10px;
            margin-top: 20px;
            height: 120px;
        }
        .charge {
            width: 100%;
            margin: 10px 0;
            border-radius: 5px;
            border-color: #ddd;
            border-width: 1px;
            padding: 10px;
        }
        .btn.disabled {
            background: #ccc;
            border-radius: 30px;
            color: black;
        }
    }
    #cover-img-upload {
        height: 50px;
        text-align: left;
        margin-bottom: 20px;
        .cover-image-upload {
            max-height: 100%;
            width: auto;
        }
    }
        
</style>